...
- **Void / Shadow (accepted-in-principle 2026-07-07, underspecified).** A genuinely genre-grounded occult type (Soulrend, Wither, Decay, Necrotic across the catalogue), kept — but two of its three defining properties are parked, so it is *not* yet a complete identity:
  - **Signature — "defence-bypass" REJECTED** (per the resist-parity rule above): it is armour-penetration, an augment, and a resist-ignoring type is un-balanceable under Model A. A legal target-debuff signature must replace it; the grounded candidates are **Wither** (stacking damage-taken amp — but overlaps Lightning's Shock, distinct only by stacking, which is now Poison's trait) or **Decay** (a DoT — overlaps Burn/Poison). Neither is clean, so the exact signature is **parked** (resolve the Shock/Poison overlap).
  - **Resist channel — parked** on the roster-growth call: its own "Void" channel vs. a **shared "Chaos"/occult channel with Poison**.
  - Both parked specifics resolve together with the roster-growth decision below.
- **⚠ Roster-growth flag (open).** Adopting Poison with its own channel sets a precedent that each occult type grows the resist roster (Void likely wants its own too). **How large the resist roster is allowed to grow — and whether occult types share a "Chaos" channel — is a deliberate call deferred to when Void/Blood/Holy are assessed** — not decided per-candidate. For Poison alone, own-channel is the genre-correct answer.
...
- **Survivors → occult wave (adopted-in-principle, gated):** **Poison** (own channel + stacking-DoT; gated on stacking-EoT engine + 5th channel) and **Void/Shadow** (accepted but signature [Wither/Decay] and channel [own vs shared-Chaos] parked on the roster decision).
...
- **Downstream threads this pass hands off:** (1) the **roster-growth / shared-Chaos-channel** decision (unblocks Void's channel + signature); (2) the **occult-wave build** = stacking-EoT engine + Poison's 5th channel; (3) the D1-unblocked **fleet↔enduring and reserve buff/debuff forms**, now awaiting their own adoption pass + the `EotData` baseline-vs-modifier split. The next raw table in the forms→identities→augments order is the **augment extraction table**.
...
- **Buff auras — Phase 2 (expensive, deferred; issue #64).** A general player-buff framework (buff-def + registry + stat-calc hooks) does not exist — only an ad-hoc `_rangeBuffBonus`. With no allies (party support rejected), a buff aura is a toggled persistent **self**-buff. Split out because the framework is a substantial self-contained build; buff magnitude follows the same D1 baseline × modifier model.
