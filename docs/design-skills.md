...
- **Void (RESOLVED 2026-07-08).** A genuinely genre-grounded occult type (Soulrend, Wither, Decay, Necrotic across the catalogue). Identity is now complete:
  - **Signature — Decay: a %-of-target's-max-HP DoT, computed once at apply time (stored as a flat per-tick amount, mirroring Poison's `DamagePerTick` shape), non-stacking (`MaxStacks = 1`, refreshes in place), and does not crit-scale.** The "defence-bypass" signature was rejected earlier (per the resist-parity rule above — it is armour-penetration, an augment, and a resist-ignoring type is un-balanceable under Model A). The next candidates, **Wither** (stacking damage-taken amp) and flat-damage **Decay**, both overlapped an existing signature (Shock, Poison) too closely. Decay redefined as %-max-HP settles that: non-stacking keeps it distinct from Poison (whose sole differentiator *is* stacking), and no-crit-scaling avoids compounding a %-of-max-HP magnitude with a crit multiplier against high-HP targets — exactly the case %-max-HP scaling exists to matter most for. *Justification: a %-max-HP, non-stacking, non-crit DoT is now definitionally distinct from every other signature (Bleed/Burn/Chill/Shock/Poison) on at least one axis each, satisfying the adoption test cleanly.*
  - **Resist channel — VoidResistance**, Void's own channel (resolved 2026-07-07, see roster-growth call below), following the exact Poison/`PoisonResistance` precedent.
  - **Identity/EoT id: `"void"` / `"decay"`** — single-word, matching the existing `physical`/`fire`/`cold`/`lightning`/`poison` convention.
  - **Ticketed as #76** (Void identity — VoidResistance channel + Decay signature). Unlike Poison, this does not need the stacking-EoT engine (Decay is non-stacking), so it's a smaller ticket: new resist channel + `EotData`/`IdentityRegistry` entries + the %-max-HP-at-apply-time tick-damage computation.
- **Roster-growth call — RESOLVED (2026-07-07): each occult type earns its own resist channel, not a shared "Chaos" channel.** Poison gets its own channel; Void (and any future occult type) follows the same rule rather than pooling into a shared channel.
  ...
  *Justification: a shared channel would mean an enemy's Chaos-resistance blunts every occult type equally, flattening exactly the per-type build identity the adoption test exists to protect (the same reasoning that earns Poison its own channel over sharing Physical's). The cost — enemy resist blocks grow by one channel per occult type adopted — is accepted because occult types are gated/rare additions, not a fast-growing roster; own-channel keeps each type's resist tuning independent and matches the genre-standard treatment of Poison/Chaos as distinct from other schools.*
  ...
  This unblocks Poison outright — it no longer needs Void resolved to proceed. Void's own channel is settled too.
...
- **Survivors → occult wave:** **Poison** (own channel + stacking-DoT; shipped — #73) and **Void** (own channel + Decay signature, %-max-HP/non-stacking/non-crit; resolved — ticketed as #76).
