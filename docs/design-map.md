...
Currently, maps are procedurally generated by placing rooms and corridors on a fixed integer grid. Floor patches are built dynamically as flat `BoxMesh` regions, and invisible boundary walls keep players inside the generated map bounds. The connector-snapping modular chunk-stitching algorithm is a planned future extension.

### Grid Placement Algorithm (current)
...
3. For each additional room up to the map's room count (currently 4–6), a random existing room is selected, and a neighbor cell in one of the 4 cardinal directions is checked. If unoccupied, a new room is placed there and connected via a corridor.
...
## Future Modular Chunks (deferred)

Later, the generator will be upgraded to stitch pre-authored **chunks** — floor sections with defined connector slots on their edges — rather than using simple grid boxes.
...
| Biome | Status | Notes |
|---|---|---|
| Hollow Dark Forest | Implemented — first biome | Dark earth floor, tree trunk walls, dead wood props |
