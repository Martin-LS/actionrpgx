...
| ~~**ramp**~~ ✅ | ~~Channeled tick rate builds the longer the channel is held; drain constant~~ | self_channeled_tick (3rd form) | — | **ADOPTED 2026-07-06 → `design-skills.md`** (temporal-ramp axis; 2 combos). "Builds" = tick rate not damage (hit-size rule); power-neutral via Balancer ramp curve (situational — rewards holds, punished by kiting/interrupts). Bounded engine hook: within-channel hold-time accumulator → tick rate, reset on release; NOT the gated charge mechanic. |
| ~~**fuse**~~ ✅ | ~~Payload lands on a placement timer instead of instantly~~ | fixed_zone_burst | — | **ADOPTED 2026-07-06 → `design-skills.md`** as fixed_zone_burst's delayed-detonation form (2 combos; wind-up + long CD ↔ big radius, mirror of quake; ships on existing WindUp-on-Position machinery). **STRUCTURAL: confirmed wind-up is a form axis, not a chassis — windup_burst dissolves** (differs from fixed_zone_burst only in WindUp; quake is the wave-1 precedent). Follow-up impl issue: retire windup_burst engine-proof prototype. |
...
- **2026-07-06 (fuse synthesis)** — Eighth form-candidate pass, the structural one. **ADOPTED fuse as fixed_zone_burst's delayed-detonation form** (lobbed bomb: wind-up + long CD ↔ big radius, Position mirror of quake; 2 combos; tier track radius↑; ships on existing WindUp-on-Position machinery). **STRUCTURAL RULING: wind-up is a form axis on burst prototypes, not a delivery chassis.** windup_burst differs from fixed_zone_burst in exactly one field (WindUp), which is a budget lever — so it was never a distinct chassis; quake is the wave-1 precedent (wind-up already a form on self_burst). windup_burst does not become player-facing / gets no forms; recommend retiring it as engine-proof scaffold in a follow-up impl issue (its wind-up-proving job is done). fixed_zone_burst now carries 3 forms: blast (instant hub) / fuse (timing) / echo (granularity). Prototype roster effectively 12→11. Only residue left (Burst→Tick boundary probe). Also created follow-up: **needs a GitHub issue to retire windup_burst** (+ the salvo/echo multi-hit engine, and the ÷N proc-normalization rule — batch the implementation tickets after synthesis).
- **2026-07-06 (ramp synthesis)** — Seventh form-candidate pass. **ADOPTED as self_channeled_tick's third form** (temporal-ramp axis vs. the spin/vortex radius↔drain pair; 2 combos; tier track ramp-speed↑). "Builds" = tick rate not damage (hit-size rule's 5th application). Power-neutral via a Balancer-priced ramp curve — situational (rewards uninterrupted long holds; punished by kiting/interrupts that reset it), not a strict upgrade. Bounded engine hook: within-channel hold-time accumulator driving current tick rate, reset on release — explicitly NOT a stored-stack/charge system (that's the gated charge_release mechanic). All engine-hook candidates now cleared. Remaining: the two boundary fights — fuse (may fold windup_burst from prototype to form axis) + residue (Burst→Tick pattern-boundary probe).
