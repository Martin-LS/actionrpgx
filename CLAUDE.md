...
- MCP tools are auto-approved globally, except for running/querying the live editor — see AGENTS.md Rule 0.
- When debugging runtime behaviour (animation, physics, signals, gameplay logic): read the log via `get_output_log`/`get_editor_errors` before drawing conclusions — do not guess from code alone. This still requires approval per AGENTS.md Rule 0.
