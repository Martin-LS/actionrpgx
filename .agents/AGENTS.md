...
- **Complexity labels** (`complexity-low` / `complexity-medium` / `complexity-high`, issues only): a *routing signal for the user*, orthogonal to the Stage family — they indicate the **minimum agent capability** the issue needs (a floor: `complexity-low` = any agent may claim it, `complexity-medium` = a mid-tier or better agent, `complexity-high` = a top-tier agent only, e.g. work needing extensive Godot/Blender MCP interaction or real design judgment). Routing is entirely the user's call — no agent reads its own tier or self-gates on these; the label just lets the user see at a glance who to hand an issue to (and `worker-issues` surfaces it in its listing). **Whoever creates an issue must assign exactly one `complexity-*` label as part of creating it** — this is a soft rule like the mutually-exclusive Stage labels (nothing enforces it in GitHub). **Everything resolves upward**: if an issue carries multiple complexity labels, or none at all, treat it as `complexity-high` — the fail-safe direction, so a weaker agent never picks up work that's secretly too hard. A periodic sweep can clean up malformed (multi- or un-labeled) issues; no hard gate needed.
